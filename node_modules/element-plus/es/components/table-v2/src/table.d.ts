...
import type { Column, ColumnCommonParams, DataGetter, KeyType, RowCommonParams, SortBy, SortState } from 'element-plus/es/components/table-v2/src/types';
/**
 * Param types
 */
 ...
 */
export type ExtraCellPropGetter<T> = (params: ColumnCommonParams<T> & RowCommonParams & {
    cellData: T;
    rowData: any;
}) => any;
...
export type ExtractHeaderCellPropGetter<T> = (params: ColumnCommonParams<T> & {
    headerIndex: number;
}) => any;
export type ExtractRowPropGetter<T> = (params: {
    columns: Column<T>[];
} & RowCommonParams) => any;
...
export type RowClassNameGetter<T> = (params: {
    columns: Column<T>[];
...
export declare const tableV2Props: {
    ...
    readonly rowEventHandlers: {
        readonly type: import("vue").PropType<import("element-plus/es/components/table-v2/src/row").RowEventHandlers>;
        readonly required: false;
        readonly validator: ((val: unknown) => boolean) | undefined;
        __epPropKey: true;
    };
